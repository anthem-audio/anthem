...
/// matching the lifecycle of [ProjectController] while keeping track concerns
/// separate from broader project concerns.
class TrackController {
  ...
  void insertTrackAt(Id anchorTrackId) {
    ...
    if (anchorTrack.type == TrackType.group) {
      ...
      if (parentTrack == null) {
        throw StateError(
          'TrackController.insertTrackAt(): Parent track $parentTrackId not '
          'found for track $anchorTrackId.',
        );
      }
      ...
      if (anchorIndex == -1) {
        throw StateError(
          'TrackController.insertTrackAt(): Track $anchorTrackId not found in '
          'child list of parent track $parentTrackId.',
        );
      }
      ...
      if (anchorIndex == -1) {
        throw StateError(
          'TrackController.insertTrackAt(): Top-level track $anchorTrackId not '
          'found in ${anchorIsSendTrack ? 'sendTrackOrder' : 'trackOrder'}.',
        );
      }
